...
from rspn.algorithms.expectations import expectation_spn, expectation_nc, expectation_bn
from rspn.algorithms.ranges import NominalRange, NumericRange
from rspn.algorithms.validity.validity import is_valid
from rspn.learning.rspn_learning import learn_mspn, learn_mnc, learn_mbn
...
class RSPN:
    ...
    def __init__(self, meta_types, null_values, full_sample_size, column_names=None, table_meta_data=None, ensemble_type='spn', model_id=None):
        ...
        logging.debug(f"RSPN created column_names {self.column_names}")
    ...
    def learn(self, train_data, rdc_threshold=0.3, min_instances_slice=1, max_sampling_threshold_cols=10000,
        ...
        if self.ensemble_type == 'spn':
            ...
            logging.debug(f"Built SPN with scope {self.mspn.scope}")
        elif self.ensemble_type == 'nc':
            self.mspn = learn_mnc(train_data, ds_context, model_id=self.model_id)
            logging.debug(f"Built NC with scope {self.mspn.scope}")
        elif self.ensemble_type == 'bn':
            self.mspn = learn_mbn(train_data, ds_context, model_id=self.model_id)
            logging.debug(f"Built BN with scope {self.mspn.scope}")
    ...
    def _indicator_expectation(self, feature_scope, identity_leaf_expectation=None, inverted_features=None,
        ...
        if hasattr(self, 'use_generated_code') and self.use_generated_code and not force_no_generated:
            ...
            # This is used!
            if not hasattr(self, 'ensemble_type') or self.ensemble_type == 'spn': 
                full_result = expectation_spn(self.mspn, feature_scope, inverted_features, range_conditions,
                                              node_expectation=_node_expectation, node_likelihoods=_node_likelihoods_range)
            elif self.ensemble_type == 'nc':
                full_result = expectation_nc(self.mspn, feature_scope, inverted_features, range_conditions,
                                             node_expectation=_node_expectation, node_likelihoods=_node_likelihoods_range)
    ...
    def _unnormalized_conditional_expectation(self, feature_scope, inverted_features=None, range_conditions=None,
        ...
        if any(p == 0):
            ...
            e = self._indicator_expectation(feature_scope, inverted_features=inverted_features,
                                               gen_code_stats=gen_code_stats)
            logger.debug(f'RETURN UNNORMALIZED CONDITIONAL EXP {e}')
            return e

        logger.debug(f'RETURN UNNORMALIZED CONDITIONAL EXP {unnormalized_exp / p}')
    ...
    def _normalized_conditional_expectation(self, feature_scope, inverted_features=None, normalizing_scope=None,
        ...
        if normalizing_scope is None or len(normalizing_scope) == 0:
            if standard_deviations:
                ...
                e = self._unnormalized_conditional_expectation(feature_scope,
                                                               inverted_features=inverted_features,
                                                               range_conditions=range_conditions,
                                                               impute_p=impute_p,
                                                               gen_code_stats=gen_code_stats)
                logger.debug(f'RETURN NORMALIZED CONDITIONAL EXP {e}')
                return None, e
        ...
        logger.debug(f'GOT NOMINATOR = {nominator}')
        ...
        denominator = self._indicator_expectation(normalizing_scope,
                                                  ...
                                                  range_conditions=range_conditions)
        logger.debug(f'GOT DENOMINATOR = {denominator}')
        logger.debug(f'RETURN NORMALIZED CONDITIONAL EXP {nominator / denominator}')

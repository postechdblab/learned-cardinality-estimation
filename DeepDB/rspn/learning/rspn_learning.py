...
from naru.train_model import TrainNARU, TrainBN
...
def learn_mnc(
    ...
    '''
     to learn NARU using data, save data with a simple header [0,1,2,3,...,n]
     load NARU and test simple queries
     after loading, make a test {cols, ops, vals} and call est._sample_n used in est.Query(cols, ops, vals)
    '''

    df = pd.DataFrame(data)

    logger.debug(f'starting model {model_id}')
    #table_path = f'naru/datasets/custom{model_id}.csv'
    model_path = f'naru/models/naru_custom{model_id}.pt'

    # write data to table_path
    #logger.debug(f'writing data {model_id} with size {data.shape} to {table_path}')
    #data.to_csv(table_path, header=None, index=False)
    #np.savetxt(table_path, data, header='', delimiter=',')
    #data.to_csv(table_path, header=None, index=False)
    #logger.debug(f'writing done {model_id}')

    # train NARU
    logger.debug(f'training NARU {model_id}')
    est = TrainNARU(model_id, df, model_path)
    logger.debug(f'training done {model_id}')

    # load NARU
    #logger.debug(f'loading NARU {model_id}')
    #est = GetNARU(model_id, table_path, model_path)
    #logger.debug(f'loading done {model_id}')
    
    # move NARU from GPU to CPU
    logger.debug(f'moving NARU {model_id}')
    est.MoveToCPU()
    logger.debug(f'moving done {model_id}')

    
    """
    logger.debug(f'running NARU')
    # generate a test query
    from naru.eval_model import GenerateQuery
    cols, ops, vals = GenerateQuery(est.table.columns, np.random.RandomState(1234), est.table)

    # run
    est_card = est.Query(cols, ops, vals, X_feature_scope=[0,1,2,3,4], X_inverted_features=[False,False,False,False,False])
    logger.debug(f'running done')
    """

    logger.debug(f'est.scope {model_id} {est.scope}, scope {scope} length {len(est.scope)}')
    assert est.scope == scope
    assert len(est.scope) == data.shape[1]
    logger.debug(f'ending model {model_id}')
...
def learn_mbn(
    ...
    logger.debug(f'starting model {model_id}')
    model_path = f'naru/models/bn_custom{model_id}.pt'

    # train BN 
    logger.debug(f'training BN {model_id}')
    est = TrainBN(model_id, df, model_path)
    logger.debug(f'training done {model_id}')

...
def read_nc_ensemble(ensemble_locations, build_reverse_dict=False):
    """
    ...
    for ensemble_location in ensemble_locations:
        with open(ensemble_location, 'rb') as handle:
            ...
            for spn in current_ensemble.spns:
                logging.debug(f"Including NC with table_set {spn.table_set} with sampling ratio"
...
class NCEnsemble:
    ...
    - SPNs represent trees. XXX should break
    ...
    def cardinality(self, query, rdc_spn_selection=False, pairwise_rdc_path=None,
        ...
        # max_variants is always 1 as default
    ...
    def _cardinality_with_injected_start(self, query, first_spn, next_mergeable_relationships, next_mergeable_tables,
        ...
        # First SPN: Full_join_size*E(outgoing_mult * 1/multiplier * 1_{c_1 Λ ... Λ c_n})
        ...
        # E(1/multipliers * 1_{c_1 Λ ... Λ c_n})

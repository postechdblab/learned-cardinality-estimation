...
class AQPSPN(CombineSPN, RSPN):
    ...
    def __init__(self, meta_types, null_values, full_join_size, schema_graph, relationship_list, full_sample_size=None,
                 table_set=None, column_names=None, table_meta_data=None, ensemble_type='spn', model_id=None):
        ...
        RSPN.__init__(self, meta_types, null_values, full_sample_size,
                      ...
                      ensemble_type=ensemble_type, model_id=model_id)
    ...
    def evaluate_indicator_expectation_batch(self, indicator_expectation, group_bys, group_by_tuples,
        ...
        range_conditions = self._parse_conditions(indicator_expectation.conditions, group_by_columns=group_bys,
                                                  group_by_tuples=group_by_tuples)
        if range_conditions is None:
            logger.info(f"range_conditions None")
            return None, None
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None):
        ...
        for (table, condition) in conditions:
            ...
            matching_fd_cols = [column for column in sorted(list(self.table_meta_data[table]['fd_dict'].keys()), key=len)
                                if column in table + '.' + condition]
            matching_cols = [column for column in sorted(self.column_names, key=len) 
                             if column in table + '.' + condition]
            # leave the longest-matching column
            if len(matching_cols) > 1:
                matching_cols = [matching_cols[-1]]
            if len(matching_fd_cols) > 1:
                matching_fd_cols = [matching_fd_cols[-1]]
            if len(matching_cols) != 1 and len(matching_fd_cols) != 1:
                return None
            ...
            if len(matching_cols) == 1:
                ...
                if '=' in condition:
                    ...
                    assert False, "LIKE or NOT LIKE is in wrong condition"
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                if '!=' in condition:
                    ...
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        val_list = [v for k,v in val_dict.items() if k != literal and not isNaN(k)]
                        ranges[:, attribute_index] = NominalRange(val_list)
                    else:
                        assert False, 'Not implemented != operator in groupby'
                    ...
                    not_in_value_list = [val_dict[literal] for literal in val_dict.keys() if not literal in literal_list and not isNaN(literal)]
                    if self.null_values[attribute_index] in not_in_value_list:
                        not_in_value_list.remove(self.null_values[attribute_index])
                    single_range = NominalRange(not_in_value_list)
                    ...
                    literal_list = _literal_list(condition)
                    single_range = NominalRange([val_dict[literal] for literal in literal_list if literal in val_dict.keys()])
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                ...
                elif 'NOT LIKE' in condition:
                    column, pattern = condition.split('NOT LIKE', 1)
                    ...
                    single_range = NominalRange(
                        [val_dict[literal] for literal in val_dict.keys() if  type(literal)==str and not p.match(literal)])
                    ...
                    p = re.compile(pattern)
                    single_range = NominalRange(
                        [val_dict[literal] for literal in val_dict.keys() if  type(literal)==str and p.match(literal)])
                    ...
                    single_range = NominalRange([val_dict[literal] for literal in val_dict.keys() if not isNaN(literal)])
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                    ...
                    single_range = NominalRange([val_dict[literal] for literal in val_dict.keys() if isNaN(literal)])
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                ...
                if '<=' in condition:
                    ...
                    literal = float(literal.strip()) 
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        ranges[:, attribute_index] = NumericRange([[-np.inf, literal],[literal,np.inf]],inclusive_intervals=[[False,False],[False,False]])
                    else:
                        matching_group_by_idx = group_by_columns_merged.index(matching_column)
                        assert literal == group_by_tuples[0][matching_group_by_idx]
                        for idx in range(len(ranges)):
                            literal = group_by_tuples[idx][matching_group_by_idx]
                            ranges[idx, attribute_index] = NumericRange([[-np.inf, literal],[literal,np.inf]],inclusive_intervals=[[False,False],[False,False]])
                    ...
                    literal_list = _literal_list(condition)
                    single_range = _notin_ranges(literal_list,inclusive=False)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, numeric_range in enumerate(ranges[:, attribute_index]):
                            ranges[i, attribute_index] = NumericRange(list(set(numeric_range.ranges).intersection(single_range.ranges)))

                elif 'IN' in condition:
                    literal_list = _literal_list(condition)
                    in_range_list = [ [float(literal.strip()),float(literal.strip())] for literal in literal_list]
                    ...
                    map(non_conflicting, ranges)
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        ranges[:, attribute_index] = NumericRange(in_range_list)
                    else:
                        assert False,'Not support IN condition in the groupby'
                            
                elif 'IS NOT NULL' in condition:
                    single_range = NumericRange([[-np.inf,np.inf]],is_not_null_condition=True)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                            ranges[i, attribute_index] = NumericRange(list(
                                set(numeric_range.possible_values).intersection(single_range.possible_values)))
                elif 'IS NULL' in condition:
                    single_range = NumericRange([[np.nan,np.nan]],null_value=np.nan,is_not_null_condition=False)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
...
def _notin_ranges(literal_str_list,inclusive=False):
    
    data_list = sorted([float(literal.strip()) for literal in literal_str_list])
    
    if len(data_list) == 1:
        data = data_list[0]
        literal_list = [[-np.inf,data],[data,np.inf]]
        inclusive_interval_list = [[False, inclusive], [inclusive, False]]

    else:
        literal_list = list()
        inclusive_interval_list = list()        
        for i,data in enumerate(data_list):
            if i == 0: 
                literal_list.append([-np.inf,data])
                inclusive_interval_list.append([False,inclusive])
            else:           
                prev_data = data_list[i-1]
                literal_list.append([prev_data,data])
                inclusive_interval_list.append([inclusive,inclusive])
        
        literal_list.append([data,np.inf])
        inclusive_interval_list.append([inclusive,False])
    return NumericRange(literal_list,inclusive_intervals=inclusive_interval_list) 

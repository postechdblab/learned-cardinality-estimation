...
from FullOuterSampler.factorized_sampler_lib import data_utils
from FullOuterSampler.factorized_sampler_lib import rustlib
...
#         print(f"jct ret {table} dtype : {ret.fillna(1).dtypes} to int\n{ret.fillna(1).head(5)}")
...
def get_join_count_tables(join_spec,df_dict):
#     print(f"JOIN SPEC: {[v for k,v in join_spec.join_keys.items()]}")
#     print(f"DF_DICT: {[list(df.columns) for k,df in df_dict.items()]}")

    base_count_tables_dict = {
        # +@ pass parameter
        table: get_base_count_table.remote(join_spec.join_name, table, keys, df_dict)
...
# +@ pass dir, dataset, use_col_name
@ray.remote
def get_base_count_table(join_name, table, keys, df_dict):
    ...
    # +@ fix load
    def work(table, keys):
        df = data_utils.load_table(table,df_dict = df_dict,
                                   dtype={k: pd.Int64Dtype() for k in keys})
#         print(f"table {table} has group by keys = {keys} where df has columns {df.columns}")
        for k in keys:
        ...
        for key in keys:
            ...
            # +@ 0729 add for null value count - it can prevent type cast err
#             bct[f"{key}.cnt"] = bct[f"{key}.cnt"].fillna(1)
#         print(f"bct {table} dtype : {bct.dtypes} to int\n{bct.head(5)}")
...
#     print(f"get join key groups {table}\nindices({len(indices)}) : {indices[:5]}\nweights({len(weights)}) : {weights[:5]}")
...
#             print(f"get priky {table}.{col_name} dtype : {df.dtypes} to int\n{df.head(5)}")
...
# +@ pass parameters
@ray.remote
def load_data_table(table, join_keys,df_dict ):
    return data_utils.load_table(table,df_dict=df_dict,
...
def prepare(join_spec,df_dict):
    ...
    # +@ pass parameters
    jcts = get_join_count_tables(join_spec,df_dict)
    dts = {
        # +@ pass parameters
        table: load_data_table.remote(table, keys, df_dict)
        for table, keys in join_spec.join_keys.items()
    }
    jk_groups_weights = {

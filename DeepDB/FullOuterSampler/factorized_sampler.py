...
import FullOuterSampler.factorized_sampler_lib.data_utils as data_utils
import FullOuterSampler.factorized_sampler_lib.prepare_utils as prepare_utils
import FullOuterSampler.factorized_sampler_lib.rustlib as rustlib
...
def get_distribution(series):
    ...
    total = np.sum(arr)
    assert total > 0
#     print(f"arr :{arr} \ttotal :{total}\t arr/total :{arr/total}")
    arr_result = np.array(arr)
    result = arr/total
#     print(f"{np.extract(arr_result <0, arr_result)}")
#     print(f"{np.extract(result <0, result)}")
#     print(f"{series.dtype}, {len(arr)}")
    return arr / total
...
class FactorizedSampler(object):
    ...
    def __init__(self,
        ...
        prepare_utils.prepare(join_spec,df_dict)

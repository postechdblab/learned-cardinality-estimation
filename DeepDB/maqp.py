...
from schemas.imdb.schema import gen_job_light_imdb_schema, gen_job_union_schema, gen_imdb_full_schema, gen_job_original_schema
from schemas.tpc_ds.schema import gen_1t_tpc_ds_schema, gen_tpcds_benchmark_schema, gen_tpcds_full_schema
from schemas.syn.schema import gen_syn_single,gen_syn_multi,get_syn_multi_table_subset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--build_spn', help='Build SPN?', action='store_true')
    parser.add_argument('--build_naru', help='Build NARU?', action='store_true')
    parser.add_argument('--use_jct', help='Use JCT?', action='store_true')
    parser.add_argument('--join_key_as_predicate', help='Allow predicates on join keys?', action='store_true')

    parser.add_argument('--outer_sample', action='store_true')
    parser.add_argument('--load_rdc', action='store_true')
    ...
    elif args.dataset == 'job-union':
        schema = gen_job_union_schema(table_csv_path)
    elif args.dataset == 'imdb-full': # modi
        schema = gen_imdb_full_schema(table_csv_path)
    elif args.dataset == 'job-original':
        schema = gen_job_original_schema(table_csv_path)
    elif args.dataset == 'syn-single':
        schema = gen_syn_single(table_csv_path)
    elif args.dataset == 'syn-multi':
        schema = gen_syn_multi(table_csv_path)
    elif args.dataset == 'tpcds-benchmark':
        schema = gen_tpcds_benchmark_schema(table_csv_path)
    elif args.dataset == 'tpcds-full':
        schema = gen_tpcds_full_schema(table_csv_path)
    else:
    ...
    if args.generate_sampled_hdfs:
        ...
        prepare_sample_hdf(schema, args.hdf_path, args.max_rows_per_hdf_file, args.hdf_sample_size,args.outer_sample)
        logger.info(f"Files successfully created")
    ...
    if args.generate_ensemble:
        ...
        if not os.path.exists(args.ensemble_path + '/spn_ensemble'):
            os.makedirs(args.ensemble_path + '/spn_ensemble')
        if not os.path.exists(args.ensemble_path + '/nc_ensemble'):
            os.makedirs(args.ensemble_path + '/nc_ensemble')
        ...
        if args.ensemble_strategy == 'single':
            ...
            candidate_evaluation(schema, args.hdf_path, args.samples_rdc_ensemble_tests, args.samples_per_spn,
                                 ...
                                 build_spn=args.build_spn,
                                 build_naru=args.build_naru,
                                 use_jct=args.use_jct,
                                 join_key_as_predicate=args.join_key_as_predicate,
                                 outer_sample=args.outer_sample,
                                 load_rdc=args.load_rdc
                                 )
        else:
    ...
    if args.code_generation:
        spn_ensemble = read_spn_ensemble(args.ensemble_path, build_reverse_dict=True)
    ...
    if args.evaluate_cardinalities:
        ...
        evaluate_cardinalities(args.ensemble_location, args.database_name, args.query_file_location, args.target_path,
                               ...
                               meta_path =args.hdf_path +'/meta_data.pkl')

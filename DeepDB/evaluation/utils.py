...
def parse_query(query_str,schema,meta_data):
    
    def check_query_support(table,col,meta):
        if col not in meta['relevant_attributes']:
            logger.info(f"parsing check {table} {col}  not in {meta['relevant_attributes']}")
            return False
        return True
    ...
    if identifier_token_length == 3:
        ...
        # (title AS t) 
        tables = [(token[0].value, token[4].value) for token in identifiers if
                  isinstance(token, sqlparse.sql.Identifier)]
    else:
    ...
    for table, alias in tables:
        ...
        if table not in meta_data.keys() or table not in schema.table_dictionary.keys():
            return None
#     print(alias_dict)
    ...
    for in_idx in in_statements:
        ...
        if identifier.normalized =='NOT': # NOT IN query 
            identifier = where_statements.tokens[in_idx - 4]
            if len(identifier.tokens) == 1:

                left_table_name, left_attribute = _fully_qualified_attribute_name(identifier, schema, alias_dict,
                                                                                  return_split=True)
                if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                    return None
                query.add_where_condition(left_table_name, left_attribute + ' NOT IN ' + possible_values.value)


            else:
                assert identifier.tokens[1].value == '.', "Invalid identifier."
                left_table_name,left_attribute = alias_dict[identifier.tokens[0].value],identifier.tokens[2].value 
                if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                    return None
                query.add_where_condition(left_table_name, left_attribute + ' NOT IN ' + possible_values.value)
        else :
            assert isinstance(identifier, sqlparse.sql.Identifier), (identifier.normalized)
    ...
    not_null_statements = [idx for idx, token in enumerate(where_statements) if token.normalized == 'NOT NULL']
    for nn_idx in not_null_statements:
        assert where_statements.tokens[nn_idx - 1].value == ' '
        assert where_statements.tokens[nn_idx + 1].value == ' '
        if where_statements.tokens[nn_idx - 2].normalized == 'IS':
            assert where_statements.tokens[nn_idx - 3].value == ' '
            identifier = where_statements.tokens[nn_idx - 4]
            if len(identifier.tokens) == 1:
                left_table_name, left_attribute = _fully_qualified_attribute_name(identifier, schema, alias_dict,
                                                                                  return_split=True)
                if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                    return None
                query.add_where_condition(left_table_name, left_attribute + ' IS NOT NULL')
    ...
    null_statements = [idx for idx, token in enumerate(where_statements) if token.normalized == 'NULL']
    for nl_idx in null_statements:

        assert where_statements.tokens[nl_idx - 2].normalized == 'IS'
        identifier = where_statements.tokens[nl_idx - 4]
        if len(identifier.tokens) == 1:
            left_table_name, left_attribute = _fully_qualified_attribute_name(identifier, schema, alias_dict,
                                                                              return_split=True)
            if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                return None
            query.add_where_condition(left_table_name, left_attribute + ' IS NULL')
    ...
    like_statements = [idx for idx, token in enumerate(where_statements) if token.normalized == 'LIKE']
    for lk_idx in like_statements:
        assert where_statements.tokens[lk_idx - 1].value == ' '
        assert where_statements.tokens[lk_idx + 1].value == ' '
        like_pattern = ''
        like_val_idx = lk_idx + 2
        while len(where_statements.tokens)>like_val_idx and where_statements.tokens[like_val_idx].value != ' ':
            like_pattern += where_statements.tokens[like_val_idx].value
            like_val_idx+=1


        if where_statements.tokens[lk_idx - 2].normalized == 'NOT':
            assert where_statements.tokens[lk_idx - 3].value == ' ' 
            identifier = where_statements.tokens[lk_idx - 4]
            if len(identifier.tokens) == 1:
                left_table_name, left_attribute = _fully_qualified_attribute_name(identifier, schema, alias_dict,
                                                                                  return_split=True)
                if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                    return None
                query.add_where_condition(left_table_name, left_attribute + ' NOT LIKE ' + like_pattern)
            ...
            identifier = where_statements.tokens[lk_idx - 2]
            if len(identifier.tokens) == 1:
                left_table_name, left_attribute = _fully_qualified_attribute_name(identifier, schema, alias_dict,
                                                                                  return_split=True)
                if not check_query_support(left_table_name,left_attribute,meta_data[left_table_name]):
                    return None
                query.add_where_condition(left_table_name, left_attribute + ' LIKE ' + like_pattern)
    ...
    for comparison in comparisons:
        ...
        if len(left.tokens) == 1:
            ...
            if isinstance(right, sqlparse.sql.Identifier):
                ...
                if not check_query_support(right_table_name,right_attribute,meta_data[left_table_name]):
                    return None
                ...
                query.fd_join_list.append(f"{left_part}={right_part}")
            ...
            if isinstance(right, sqlparse.sql.Identifier):
                ...
                assert comparison.tokens[operator_idx].value == '=', "Invalid join condition"
                query.fd_join_list.append(f"{left_part}={right_part}")
                if left_part + ' = ' + right_part not in schema.relationship_dictionary.keys() and \
                        right_part + ' = ' + left_part not in schema.relationship_dictionary.keys():
                    continue

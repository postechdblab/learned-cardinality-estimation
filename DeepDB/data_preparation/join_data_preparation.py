...
from FullOuterSampler.sampler import get_full_outer_sampler
...
class JoinDataPreparator:
    ...
    def _find_start_table(self, relationship_list, min_start_table_size):
        ...
        logger.debug(f'sampled_tables: {sampled_tables}, rates: {[self._sampling_rate(table) for table in table_set]}')
    ...
    def generate_n_samples(self, sample_size, post_sampling_factor=30, single_table=None, relationship_list=None,
        ...
        if sample_size_estimate > sample_size:
            ...
            df_full_samples, meta_types, null_values, _ = self.generate_join_sample(
        ...
        df_full_samples, meta_types, null_values, _ = self.generate_join_sample(single_table=single_table,
                                                                             relationship_list=relationship_list,
                                                                             min_start_table_size=min_start_table_size,
                                                                             sample_rate=1.0,
                                                                             drop_redundant_columns=drop_redundant_columns)
    ...
    def generate_n_samples_with_incremental_part(self, sample_size, post_sampling_factor=30, single_table=None, relationship_list=None,
                           min_start_table_size=1, drop_redundant_columns=True, incremental_learning_rate=0, incremental_condition=None, use_jct=False, join_key_as_predicate=False):
        """
        Generates approximately sample_size samples of join.
        :param sample_size:
        :param post_sampling_factor:
        ...
        if use_jct:
            ...
            sample_size_estimate, full_join_size = self._size_estimate(single_table=single_table,
                                                                       relationship_list=relationship_list,
                                                                       min_start_table_size=min_start_table_size)
        logger.debug(f"generate_n_samples_with_incremental_part(sample_size={sample_size}, single_table={single_table}, relationship_list={relationship_list}, sample_size_estimate={sample_size_estimate}, incremental_learning_rate={incremental_learning_rate}, incremental_condition={incremental_condition})")
        # Sampling of join necessary
        sample_rate = 1.0
        if sample_size_estimate > sample_size:
            sample_rate = min(sample_size / sample_size_estimate * post_sampling_factor, 1.0)
        logger.debug(f"to many samples, reduce number of samples with sample_rate={sample_rate}")
        df_full_samples, meta_types, null_values, temp = self.generate_join_sample(single_table=single_table,
                                                                             relationship_list=relationship_list,
                                                                             min_start_table_size=min_start_table_size,
        ...
        logger.debug(f"AFTER generate_join_sample, df_full_samples.columns is {df_full_samples.columns}")
        if temp is not None:
            full_join_size = temp
            logging.info(f"FULL JOIN SIZE = {full_join_size}")
        ...
        if incremental_learning_rate > 0:
            ...
            if (value is not None):
                ...
                logger.info(f"splitting dataset into {len(df_learn_samples)}:{len(df_inc_samples)} parts, according to condition ({incremental_condition}), incremental_rate: {100.0*len(df_inc_samples)/len(df_full_samples)}% @@@")
            else:
                logging.info("Currently only '<' operator is supported for incremental_condition (i.e. title.production_year<2015)")
                sys.exit(1)
        else:
            df_inc_samples = pd.DataFrame([])
            df_learn_samples = df_full_samples
        logger.info(f"split full sample dataset into parts: initial learning size: {len(df_learn_samples)}, incremental: {len(df_inc_samples)}")
    ...
    def generate_join_sample(self, single_table=None, relationship_list=None, min_start_table_size=1, sample_rate=1,
                             ...
                             split_condition=None, sample_size=None, use_jct=False, join_key_as_predicate=False):
        """
        ...
        logging.info(f"generate_join_sample(single_table={single_table}, relationship_list={relationship_list}, split_condition={split_condition})")
        if single_table is not None:

            df_samples = self._get_table_data(self.table_meta_data[single_table]['hdf_path'], single_table)
            assert df_samples.shape[0] > 0
            assert df_samples.shape[1] > 0

            if use_jct:
                table_size = len(df_samples)

            if sample_rate < 1:
                df_samples = df_samples.sample(prob_round(len(df_samples) * sample_rate))
            
            logger.info(f"df_samples.columns = {df_samples.columns}")
            ...
            if drop_redundant_columns:
                del_mul_attributes = [col for col in del_mul_attributes if col in df_samples.columns]
                logger.info(f"del_mul_attributes = {del_mul_attributes}")
                if len(del_mul_attributes)>0:
            ...
            for pk_attribute in table_obj.primary_key:
                ...
                if id_attribute in df_samples.columns:
                    if drop_redundant_columns:
                        id_attribute = [col for col in id_attribute if col in df_samples.columns]
                        logger.info(f"id_attribute = {id_attribute}")
            ...
            if drop_redundant_columns:
                del_fk_cols = [col for col in del_fk_cols if col in df_samples.columns]
                logger.info(f"del_fk_cols = {del_fk_cols}")
                if len(del_fk_cols)>0:
            ...
            if use_jct:
                return df_samples, meta_types, null_values, table_size
            else:
                return df_samples, meta_types, null_values, None
            ...
            logging.info(f"reading first table '{start_table}' with start_table_sample_rate {start_table_sample_rate}")
            df_samples = None
            # use JCTs  
            if use_jct:
                start_df = self._get_table_data(self.table_meta_data[start_table]['hdf_path'], start_table)
                start_df.columns = map(lambda s: s.replace(start_table, "", 1)[1:], start_df.columns)
                df_dict = {start_table: start_df}
                join_clause_list = []
                logging.info(f"start table {start_table} has columns {start_df.columns} and rows {len(start_df)}")
            else:
                df_samples = self._get_table_data(self.table_meta_data[start_table]['hdf_path'], start_table)
                if sample_rate < 1:
                    df_samples = df_samples.sample(prob_round(len(df_samples) * sample_rate))
                logging.info(f"start table {start_table} has columns {df_samples.columns} and rows {len(df_samples)}")
            ...
            while len(todo_relationships) > 0:
                ...
                logging.info(f"Joining {relationship_obj.identifier}. Current join size is {0 if use_jct else len(df_samples)}.")
                ...
                if outgoing:
                    ...
                    next_table_data = self._get_table_data(self.table_meta_data[next_joined_table]['hdf_path'],
                                                           next_joined_table)
                    logging.info(f"outgoing next_table_data = {next_joined_table}, {next_table_data.columns}")
                    ...
                    # use JCTs  
                    if use_jct:
                        next_table_data.columns = map(lambda s: s.replace(next_joined_table, "", 1)[1:], next_table_data.columns)
                        logging.info(f"next table {next_joined_table} has columns {next_table_data.columns} and rows {len(next_table_data)}")
                        df_dict[next_joined_table] = next_table_data
                        join_clause_list.append(f"{left_attribute}={right_attribute}")
                    else:
                        ...
                        logging.info(f"JOINED SAMPLES = {df_samples.columns}, {len(df_samples)}")
                ...
                # incoming edge, e.g. date joined, join lineorders
                    ...
                    next_table_data = self._get_table_data(self.table_meta_data[next_joined_table]['hdf_path'],
                                                           next_joined_table)
                    logging.info(f"incoming next_table_data = {next_joined_table}, {next_table_data.columns}")
            ...
            if use_jct:
                ...
                assert len(df_dict) == len(join_clause_list) + 1
                sampler, _ = get_full_outer_sampler(df_dict, join_clause_list, sample_bs=sample_size)
                full_join_size = sampler.join_card
                logging.info(f"FULL JOIN SIZE INITIAL {full_join_size}")
                df_samples = sampler.run()
                logging.info(f"FINAL SAMPLE HAS COLUMNS {df_samples.columns}")
                cur_table = ''
                new_column_names = []
                for name in df_samples.columns:
                    if name.startswith('mul_'):
                        assert len(name.split('.')) == 2
                        new_column_names.append(cur_table + '.' + name)
                    else:
                        assert len(name.split('.')) == 2
                        cur_table = name.split('.')[0]
                        new_column_names.append(name)
                df_samples.columns = new_column_names
                logging.info(f"FINAL SAMPLE NEW COLUMNS {df_samples.columns}")
            else:
                if len(df_samples) > max_intermediate_size:
                    df_samples = df_samples.sample(max_intermediate_size)
            ...
            logging.info(f"AFTER drop_redundant_columns, df_samples.columns is {df_samples.columns}")
            ...
            for table in joined_tables:
                ...
                # DeepDB originally assumed no join_key_as_predicate, so just "replace" ID column to nn_attribute
                # however, with join_key_as_predicate, we should "add" nn_attribute and keep ID column
                nn_set = True
                if df_samples[id_attribute].isna().any():
                    if join_key_as_predicate:
                        df_samples[nn_attribute] = 0 if not nn_set else df_samples[id_attribute]
            ...
            if drop_redundant_columns:
                del_id_columns = [col for col in del_id_columns if col in df_samples.columns]
                if len(del_id_columns)>0:
                    df_samples = df_samples.drop(columns=del_id_columns)
                logging.info(f"AFTER del_id_columns, df_samples.columns is {df_samples.columns}")
            ...
            if drop_redundant_columns:
                ...
                if len(del_fk_cols)>0:
                    df_samples = df_samples.drop(columns=del_fk_cols)
                logging.info(f"AFTER del_fk_cols, df_samples.columns is {df_samples.columns}")
            ...
            logging.info(f"Final join size is {len(df_samples)}.")

            if use_jct:
                return df_samples, meta_types, null_values, full_join_size
            else:
                return df_samples, meta_types, null_values, None


def prepare_sample_hdf(schema, hdf_path, max_table_data, sample_size,outer_sample=False):
        ...
        for relationship_obj in schema.relationships:
            if (relationship_obj.start in sampled_tables and not relationship_obj.end in sampled_tables) or (
                ...
                if relationship_obj.start in sampled_tables and not relationship_obj.end in sampled_tables:
                    ...
                    left_attribute = relationship_obj.end + '.' + relationship_obj.end_attr
                    right_attribute = relationship_obj.start + '.' + relationship_obj.start_attr
                    sample_table = relationship_obj.start
                    df_samples = df_sample_cache[relationship_obj.start]
                    df_samples = df_samples.set_index(right_attribute, drop=False)
                    df_samples.index.name = None
                    next_table_data = next_table_data.set_index(left_attribute, drop=False)
                    logging.info(f"df_samples columns = {df_samples.columns}, next_table_data columns = {next_table_data.columns}, df_samples index = {df_samples.index.name}, next_table_data index = {next_table_data.index.name}")

                    # only keep rows with join partner
                    merge_data = df_samples.merge(next_table_data, right_index=True, left_on=right_attribute,copy=True)
                    if outer_sample and len(merge_data) == 0:
                        merge_data = df_samples.merge(next_table_data, right_index=True, left_on=right_attribute,how='outer')
                        logging.info(f"Use outer-join sample : {sample_table} - {next_joined_table} -> {len(merge_data)}")
                    next_table_data = merge_data
                    ...
                    left_attribute = relationship_obj.end + '.' + relationship_obj.end_attr
                    right_attribute = relationship_obj.start + '.' + relationship_obj.start_attr
                    sample_table = relationship_obj.end
                    ...
                    df_samples.index.name = None
                    next_table_data = next_table_data.set_index(right_attribute, drop=False)
                    merge_data = df_samples.merge(next_table_data, right_index=True, left_on=left_attribute,copy=True)
                    if outer_sample and len(merge_data) == 0:
                        merge_data = df_samples.merge(next_table_data, right_index=True, left_on=left_attribute,how='outer')
                        logging.info(f"Use outer-join sample : {sample_table} - {next_joined_table} -> {len(merge_data)}")

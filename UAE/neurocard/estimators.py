...
class CardEst(object):
    ...
    def report(self):
        ...
        if est.name == 'CardEst':
            est.name = str(est)
        assert len(est.errs) > 0, len(est.errs)
                ...
                if col.factor_id is None:
                    ...
                    if o == 'IN':
                        ...
                        if len(v) == 0:
                            ...
                            p_v_list = np.vectorize(col.ProjectValue)(val_list)
                            fact_vals[i].append(tuple(p_v_list))
                            fact_dominant_ops[i].append('NOT_IN')
                    ...
                    #   everything.
                        ...
                        if o in ['<=', '<', '!='] and np.any(
                            ...
                            fact_dominant_ops[i].append(
                                col.ProjectOperatorDominant('>'))
                        #TODO for o not in =/!=, change v to relevant v' that appears in DB
                        if v not in columns[j].all_distinct_values:
                            ...
                            if o == '=':
                                ...
                                # print(columns[j].all_distinct_values)
                                #if len(columns[j].all_distinct_values) > 0 and np.isnan(columns[j].all_distinct_values[0]) and columns[j].all_distinct_values.dtype.type == np.string_:
                                #    columns[j].all_distinct_values[0] = ''
                                f = columns[j].all_distinct_values[0]
                                if type(v) == str and type(f) == float and np.isnan(f):
                                    print('if string type and nan')
                                    #columns[j].all_distinct_values[np.isnan(columns[j].all_distinct_values)] = ''
                                    columns[j].all_distinct_values[0] = str('')
                                # print(type(columns[j].all_distinct_values[0]))
                                # print(type(columns[j].all_distinct_values[1]))
                                if o == '<' or o == '>=':
                                    value = np.nonzero(columns[j].all_distinct_values >= v)[0][0]
                                    #value = np.nonzero(np.greater_equal(np.columns[j].all_distinct_values, v))[0][0]
                                elif o == '>' or o == '<=':
                                    value = np.nonzero(columns[j].all_distinct_values <= v)[0][-1]
                                    #value = np.nonzero(np.less_equal(columns[j].all_distinct_values, v))[0][-1]
                                p_v = col.ProjectValue(value)
                                p_op = col.ProjectOperator(o)
                                p_dom_op = col.ProjectOperatorDominant(o)
                                # print('new p_v: ', p_v)
                                # print('new p_op: ', p_op)
                                # print('new p_dom_op: ', p_dom_op)
...
class ProgressiveSampling(CardEst):
    ...
    def _scale_probs(self, columns, operators, vals, p, ordering, num_fanouts,
                     num_indicators, inp, query_driven=False, train=False):
        ...
        if len(indexes_to_scale) == 0:
            return p.mean() if train else p.mean().item()
        ...
        for natural_index in sorted_indexes_to_scale:
            ...
            if query_driven:
                logits_i = self.model.logits_for_col(natural_index, logits)
                scales = torch.nn.functional.gumbel_softmax(logits_i, tau= self.tau if hasattr(self,'tau') else 1.0, hard=True)
                assert columns[natural_index].distribution_size > 1
                assert not torch.isnan(scales).any(), f'NaN scales = {scales}'
                assert not torch.isinf(scales).any(), f'Inf scales = {scales}'

                scales[torch.isinf(scales)] = 0
                scales[torch.isnan(scales)] = 0
                onehot_to_index = torch.arange(start=0, end=columns[natural_index].distribution_size, device=self.device).view(-1, 1).to(torch.float)
                print(f'arange = {onehot_to_index.item()}')
                scales = torch.matmul(scales, onehot_to_index)
            else:
                ...
                # +@
                # fanout_prob_dict[columns[natural_index].name] = fanout_probs.to_list()
            ...
            #assert scales.shape[1] == 1, f'scales shape = {scales.shape}'
            #print(f'scales_{natural_index} shape = {scales.shape}')

            # Off-by-1 in fanout's domain: 0 -> np.nan, 1 -> value 0, 2 ->
            # value 1.
            #scales[scales == np.inf] = 0
            #scales[scales == float('inf')] = 0
            actual_scale_values = torch.clamp(scales - 1, 1)
            ...
            inp = self._put_samples_as_input(scales.view(-1, 1), inp, natural_index)

        # +@
        # logging_dir = './fanout_log'
        # os.makedirs(logging_dir,exist_ok=True)
        # now = datetime.datetime.now().strftime('%m%d_%H')
        # fanout_log_path = f'{logging_dir}/{now}.txt'
        # # ---
        #
        # with open(fanout_log_path,'at') as writer:
        #     token = list()
        #     for k,v in fanout_prob_dict.items():
        #         token.append(f"{k}:{v}")
        #     txt = ','.join(token)+'\n'
        #     writer.write(txt)
        # +@ modi
        ...
        return scaled_p.mean() if train else scaled_p.mean().item()
    ...
    def _put_samples_as_input(self,
        ...
        if not isinstance(self.model, transformer.Transformer):
            inp = inp.clone()
            if natural_idx == 0:
                #print(f'model.input_bins_encoded_cumsum[{natural_idx}] = {self.model.input_bins_encoded_cumsum[natural_idx]}')
                inp[:, :self.model.input_bins_encoded_cumsum[0]] = \
                ...
                #print(f'model.input_bins_encoded_cumsum[{natural_idx-1}] = {l}')
                #print(f'model.input_bins_encoded_cumsum[{natural_idx}] = {r}')
                inp[:, l:r] = \
    ...
    def _sample_n(self,
        ...
        # print(f"{columns} \t {operators} {vals}")
        ...
        for i in range(ncols):
            ...
            if self.shortcircuit and operators[natural_idx] is None:
                inp = self._put_samples_as_input(None, inp, natural_idx)
                data_to_encode = None
            else:
                ...
                #common.PrintTensorMemory(valid_i_list[i], 'valid_i_list')
                num_classes = len(dvs)
                ...
                #print(f"probs_{i} shape = {probs_i.shape}")
                #print(f"valid_{i} shape = {valid_i.shape}")

                #if self.gumbel:
                if query_driven:
                    assert False
                    # masked softmax for range projection
                    logits_i = self.model.logits_for_col(
                            natural_idx, logits)

                    if len(valid_i.size()) == 1:
                        negative_mask = torch.cat(logits_i.shape[0] * [(valid_i.view(1,-1) - 1) * 9999], 0)
                    else:
                        negative_mask = (valid_i - 1) * 9999 
                    #print(f"negative_mask_{i} shape = {negative_mask.shape}")
                    #print(f"logits_{i} shape = {logits_i.shape}")

                    logits_i = logits_i + negative_mask

                    # If some paths have vanished (~0 prob), assign some nonzero
                    # mass to the whole row so that multinomial() doesn't complain.
                    logits_i = logits_i.masked_fill(paths_vanished, 1.0)

                    # call gumbel softmax with hard=True
                    samples_i = torch.nn.functional.gumbel_softmax(logits_i, tau= self.tau if hasattr(self,'tau') else 1.0 , hard=True)
                    #samples_i = torch.argmax(samples_i, axis=1)
                    samples_i = torch.matmul(samples_i, torch.arange(start=0, end=num_classes, device=self.device).view(-1, 1).to(torch.float))

                else:
                    # If some paths have vanished (~0 prob), assign some nonzero
                    # mass to the whole row so that multinomial() doesn't complain.
                    #probs_i = probs_i.masked_fill_(paths_vanished, 1.0)
                    probs_i = probs_i.masked_fill(paths_vanished, 1.0)
                    ...
                    probs_i = probs_i.masked_fill(paths_vanished, 0.0)
                
                #print(f"samples_{i} = {samples_i}")
                #print(f"samples_{i} shape = {samples_i.shape}")
                #print(f"pre data_to_encode_{i} shape = {samples_i.view(-1,1).shape}")
                #print(f"mask_{i} shape = {samples_i.view(-1,).shape}")
        ...
        if len(masked_probs) > 1:
            ...
            #+@ for single, there are no predicates
            p = torch.tensor([1.]) if train else np.array([1])
        ...
        if self.do_fanout_scaling:
            ...
            return self._scale_probs(columns, operators, vals, p, ordering,
                                     ...
                                     query_driven=query_driven, train=train)
    ...
    def QueryWithGrad(self, columns, operators, vals, query_driven=True, 
                      qerror=[]):
        ...
        with torch.no_grad():
            inp_buf = torch.zeros_like(self.inp) #self.inp = self.inp.clone().zero_()
            ordering = orderings[0]
            inv_ordering = utils.InvertOrder(ordering)

        #if len(qerror) >= 1:
        #    query_driven_loss = torch.mean(torch.stack(qerror))
        #    query_driven_loss.backward()
    ...
    def Query(self, columns, operators, vals, not_support=False, query_driven=False):
        if not_support:
            self.OnStart()
            self.OnEnd()
            return -1
        # +@ maintain cur_query info
        if 'VERBOSE' in os.environ:
            self.cur_query +=1
        ...
        with torch.no_grad():
            ...
            if num_orderings == 1:
                ...
                # if os.environ.get('VERBOSE', None) == '2':
                    ...
                    # +@ add modi
                    density = p
                    card_scaling_with = f"{self.cardinality} == {p * self.cardinality}"
                    self.query_info[self.cur_query]['density'] = density
                    self.query_info[self.cur_query]['card_scaling_with'] = card_scaling_with
...
#                 print(f'samples_i contain nan? {torch.isnan(samples_i).any()}')
#                 print(f'scale_vec contain nan? {torch.isnan(scale_vec).any()}')
                ...
                normal_inp_candidate = self.model.EncodeInput(
                                                ...
                                                is_onehot=True)#self.is_training) +@ need to fix if you use this estimator for inf
            ...
            if self.shortcircuit:
                if not isinstance(self.model, transformer.Transformer):
                    wildcard_inp_candidate = self.model.EncodeInput(
                        None,
                        natural_col=natural_idx,
                        is_onehot=True)#self.is_training) # [1, encode_size of i] +@ need to fix if you use this estimator for inf
                    ...
                    normal_inp_candidate = self.model.EncodeInput(
                            data_to_encode,
                            natural_col=natural_idx,
                            is_onehot=True)#self.is_training) # [bs*num_sample, encode_size of i] +@ need to fix if you use this estimator for inf
...
#         print(f'sample_n columns shape = {len(columns)}')
...
class JoinSampling(CardEst):
    ...
    def Query(self, columns, operators, vals, not_support=False):
        if not_support:
            self.OnStart()
            self.OnEnd()
            return -1

...
class Table(object):
    ...
    def TableColumnIndex(self, source_table, col, allow_none=False):
...
'''
class QueryDrivenDataset(Dataset):

    def __init__(self, loaded_queries, oracle_cards, estimator=None): 
    ...
    def __len__(self):
        return len(self.loaded_queries)

    def __getitem__(self, i):
        cols, ops, vals = self.loaded_queries[i]
        return {'ops': ops, 'vals': vals, 'oracle': self.oracle_cards[i]}

    def Start(self):
        self.index = 0
        temp = list(zip(self.loaded_queries, self.oracle_cards))
        random.shuffle(temp)
        self.loaded_queries, self.oracle_cards = zip(*temp) 

    def GetBatch(self):
        size = len(self.loaded_queries)
        if self.index + self.query_bs > size: 
            queries = [self.loaded_queries[i] for i in range(self.index, size)]
            cards   = [self.oracle_cards[i] for i in range(self.index, size)]
            self.index = size
        else:
            queries = [self.loaded_queries[i] for i in range(self.index, self.index + self.query_bs)]
            cards   = [self.oracle_cards[i] for i in range(self.index, self.index + self.query_bs)]
            self.index += self.query_bs
        return queries, cards
'''

def PrintMemoryUsage(device_id=0):
    ...
    print(f'total mem: {t/1024/1024/1024}')
    print(f'reserved mem: {r/1024/1024/1024}')
    print(f'allocated mem: {a/1024/1024/1024}')
    print(f'free mem: {f/1024/1024/1024}')

def PrintTensorMemory(tensor, prefix):
    print(f'{prefix} tensor memory: {tensor.element_size() * tensor.nelement() / (1024 * 1024)}MB')

...
from mscn.model import FCN_var
...
def train(train_data, validation_data, md_dict, labels_train, labels_valid, num_epochs, batch_size, hid_units, learning_rate, logger, gpu_id, dbname):
    ...
    predicate_feat_size = get_fcn_variable_pred_enc_size(column2vec, op2vec, dbname)
    join_feat_size = len(join2vec)

    # model = SetConv(sample_feats, predicate_feats, join_feats, hid_units)
    model = FCN_var(table_feat_size, sample_feat_size, predicate_feat_size, join_feat_size, hid_units)
...
def test(model, test_file_path, md_dict, batch_size, output_path, logger, dbname, gpu_id, use_string, no_alias):
    ...
    if use_string:
        ...
        predicates_test, joins_test = encode_data_with_string_var(predicates, joins, column_min_max_vals, column2vec, op2vec, join2vec, string_columns, word_vectors_path, is_imdb)
    else:    
...
def main():
    ...
    log_path = "/dev/null"
    # log_path = "logs/fcn_var_" + exp_name + "_" + timestamp + ".log"
    ...
    if len(args.load) > 0:
        ...
        args.train, args.dbname, args.num_train, args.num_valid, args.string, True)
        ...
        model = train(train_data, validation_data, md_dict, labels_train, labels_valid, args.epochs, args.batch, args.hid, args.lr, logger, args.gpu, args.dbname)

    if args.test != "train-only":
        if args.output == None:
            test_output_path = "results/fcn_var_" + exp_name + "_" + timestamp + ".csv"
        else:

...
def predict(model, data_loader, gpu_id):
    ...
    for batch_idx, data_batch in enumerate(data_loader):
        ...
        samples, predicates, joins, targets = samples.cuda(gpu_id), predicates.cuda(gpu_id), joins.cuda(gpu_id), targets.cuda(gpu_id)
        sample_masks, predicate_masks, join_masks = sample_masks.cuda(gpu_id), predicate_masks.cuda(gpu_id), join_masks.cuda(gpu_id)
...
def train(train_data, validation_data, md_dict, labels_train, labels_valid, num_epochs, batch_size, hid_units, learning_rate, logger, gpu_id, use_string):
    ...
    if use_string:
        ...
        operand_size = 1
    predicate_feats = len(column2vec) + len(op2vec) + operand_size
...
def test(model, test_file_path, md_dict, batch_size, output_path, logger, dbname, gpu_id, use_string, no_alias):
    ...
    samples_test = encode_samples(tables, samples, table2vec, added_tables)
    if use_string:
...
def main():
    ...
    log_path = "/dev/null"
    # log_path = "logs/mscn_" + exp_name + "_" + timestamp + ".log"
    ...
    if args.test != "train-only":
        if args.output == None:
            test_output_path = "results/mscn_" + exp_name + "_" + timestamp + ".csv"
        else:

...
from mscn.data_pool import *
from mscn.model import MSCN_FCN
...
def predict(model, data_loader, gpu_id):
    ...
    for batch_idx, data_batch in enumerate(data_loader):
        features, masks, targets = data_batch
        features, masks, targets = features.cuda(gpu_id), masks.cuda(gpu_id), targets.cuda(gpu_id)
        features, masks, targets = Variable(features), Variable(masks), Variable( targets)
...
def train(train_data, validation_data, md_dict, labels_train, labels_valid, num_epochs, batch_size, hid_units, learning_rate, logger, gpu_id):
    
    dicts = md_dict["dicts"]
    min_val = md_dict["min_val"]
    max_val = md_dict["max_val"]
    feature_sizes =  md_dict["feature_sizes"]
    
    # Load training and validation data
    table2vec, column2vec, op2vec, join2vec = dicts

    assert len(feature_sizes) == len(table2vec)

    model = MSCN_FCN(feature_sizes, hid_units)
    ...
    train_time = train_time - valid_time - model_save_time
    ...
    preds_train, t_total, times = predict(model, train_data_loader, True, gpu_id)
...
def test(model, test_file_path, md_dict, batch_size, output_path, logger, dbname, gpu_id, use_string, no_alias):
    ...
    # Transform the the joins in test data to the canonical form used in the training data
    schema_joins = [k for k,v in join2vec.items()]
    joins, added_tables, invalid = transfer_to_canonical_form(schema_joins, joins)

    invalid = filter_unsupported_predicates(predicates, invalid, string_columns)
    ...
    if use_string:
        ...
        features_test, masks_test, _ = encode_data(tables, samples, predicates, joins, column_min_max_vals, table2vec, column2vec, op2vec, join2vec, string_columns, word_vectors_path, is_imdb, added_tables)
    else:
        features_test, masks_test, _= encode_data(tables, samples, predicates, joins, column_min_max_vals, table2vec, column2vec, op2vec, join2vec, added_tables)
    ...
    test_data = make_dataset(features_test, masks_test, labels_test)
...
def main():
    ...
    log_path = "/dev/null"
    # log_path = "logs/fcn_pool_" + exp_name + "_" + timestamp + ".log"
    ...
    if len(args.load) > 0:
        ...
        dicts, column_min_max_vals, min_val, max_val, labels_train, labels_valid, train_data, validation_data, feature_sizes = get_train_dataset_from_path(
        ...
        md_dict["feature_sizes"] = feature_sizes
    ...
    if args.test != "train-only":
        if args.output == None:
            test_output_path = "results/fcn_pool_" + exp_name + "_" + timestamp + ".csv"
        else:

...
def get_bitmap_wo_data(root, sample, sample_num):
    predicate = root.get_item()
    if predicate is not None and predicate['op_type'] == 'Compare':
        table_name = predicate['left_value'].split('.')[0]
        column = predicate['left_value'].split('.')[1]
        vec = []
        pattern = re.compile(r'^[a-z_][a-z0-9_]*\.[a-z][a-z0-9_]*$')
        result = pattern.match(predicate['right_value'])
        if result is None:
            dtype = sample[table_name].dtypes[column]
            for value in sample[table_name][column].tolist():
                if isSelected(value, predicate, dtype):
                    vec.append(1)
                else:
            ...
            for i in range(len(vec), sample_num):
                vec.append(0)
        elif not predicate['right_value'].split('.')[0] in sample:
        ...
        if predicate['operator'] == 'AND':
            for child in root.get_children():
                vec = get_bitmap_wo_data(child, sample, sample_num)
                bitmap = bitand(bitmap, vec)
        elif predicate['operator'] == 'OR':
            for child in root.get_children():
                vec = get_bitmap_wo_data(child, sample, sample_num)
...
def isSelected(row_value, predicate, dtype):
    if dtype == 'int64':
        none = False
        if row_value == "None" : row_value = 0
        row_value = int(row_value)
        right_value = predicate['right_value']
        if right_value == "None": right_value, none = 0, True
        value = int(float(right_value))
            ...
            if value.startswith('__LIKE__'):
                ...
                for token in pattern.split(','):
                    ...
                    if row_value == token:
                        return True
                return False
            elif value.startswith('__ALL__'):
                pattern = value.strip('__ALL__')
                pattern = pattern.strip('{}')
                for token in pattern.split(','):
                    token = token.strip('"').strip('\'')
                    if row_value == token:

...
def add_sample_bitmap_wo_data(input_path, output_path, sample, sample_num):
    with open(input_path, 'r') as ff:
        with open(output_path, 'w') as f:
            lines=ff.readlines()
            for count, plan in enumerate(lines):
                if count % 1000 == 0: print (f'{count}/{len(lines)}')
                parsed_plan = json.loads(plan)
                nodes_with_sample = []
                for node in parsed_plan['seq']:
                    bitmap_filter = []
                    ...
                    if node != None and 'condition' in node:
                        ...
                        if len(predicates) > 0:
                            ...
                            if len(predicates) > 1:
                                recover_tree(predicates[1:], root)
                            bitmap_other = get_bitmap_wo_data(root, sample, sample_num)
                    if node != None and 'condition_filter' in node:
                        predicates = node['condition_filter']
                        if len(predicates) > 0:
                            root = TreeNode(predicates[0], None)
                            if len(predicates) > 1:
                                recover_tree(predicates[1:], root)
                            bitmap_filter = get_bitmap_wo_data(root, sample, sample_num)
                    if node != None and 'condition_index' in node:
                        predicates = node['condition_index']
                        if len(predicates) > 0:
                            root = TreeNode(predicates[0], None)
                            if len(predicates) > 1:
                                recover_tree(predicates[1:], root)
                            bitmap_index = get_bitmap_wo_data(root, sample, sample_num)
                    if len(bitmap_filter) > 0 or len(bitmap_index) > 0 or len(bitmap_other) > 0:
                        bitmap = [1 for _ in range(sample_num)]
                        bitmap = bitand(bitmap, bitmap_filter)
                        bitmap = bitand(bitmap, bitmap_index)
...
def feature_extractor(input_path, out_path):
    with open(out_path, 'w') as out:
        with open(input_path, 'r') as f:
            lines = f.readlines()
            for index, plan in enumerate(lines):
                if index % 1000 == 0: print (f'{index}/{len(lines)}')
                if plan != 'null\n':

...
def remove_invalid_tokens(predicate):
    x = re.sub(r'\(\(([a-z_0-9A-Z]+)\)::text ~~ \'(((?!::text).)*)\'::text\)', r"(\1 = '__LIKE__\2')", predicate)
    x = re.sub(r'\(\(([a-z_0-9A-Z]+)\)::text !~~ \'(((?!::text).)*)\'::text\)', r"(\1 = '__NOTLIKE__\2')", x)
    x = re.sub(r'\(\(([a-z_0-9A-Z]+)\)::text <> \'(((?!::text).)*)\'::text\)', r"(\1 = '__NOTEQUAL__\2')", x)
    x = re.sub(r'\(([a-zA-Z_]+) ~~ \'(((?!::text).)*)\'::text\)', r"(\1 = '__LIKE__\2')", x)
    x = re.sub(r'\(([a-z_0-9A-Z]+) !~~ \'(((?!::text).)*)\'::text\)', r"(\1 = '__NOTLIKE__\2')", x)
    x = re.sub(r'\(([a-z_0-9A-Z]+) <> \'(((?!::text).)*)\'::text\)', r"(\1 = '__NOTEQUAL__\2')", x)
    x = re.sub(r'(\'[^\']*\')::[a-z_]+', r'\1', x)
    x = re.sub(r'\(([^\(]+)\)::[a-z_]+', r'\1', x)
    x = re.sub(r'\(([a-z_0-9A-Z\-]+) = ANY \(\'(\{.+\})\'\[\]\)\)', r"(\1 = '__ANY__\2')", x)
    x = re.sub(r'\(([a-z_0-9A-Z\-]+) <> ALL \(\'(\{.+\})\'\[\]\)\)', r"(\1 <> '__ALL__\2')", x)
    return x
        ...
        if operator_type == 'operator':
            ...
            if re.match(r'^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$', left_value) is not None:
                ...
                if relation_name is None:
                    relation = IDX_TO_TABLE[index_name]
...
def pre2seq(predicates, alias2table, relation_name, index_name):
    if ") AND (" in predicates:
        ...
        for i, token in enumerate(tokens):
            ...
            pr = remove_invalid_tokens(token)
            # print(pr)
            prs.append(pr)
        pr = " AND ".join(prs)
        # pr = "(" + "AND".join(prs) + ")"

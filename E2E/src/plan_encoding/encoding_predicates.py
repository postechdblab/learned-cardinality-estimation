...
from src.plan_encoding.meta_info import IDX_TO_TABLE

def md5_hash_string_to_int(t):
    return int(hashlib.md5(t.encode()).hexdigest(), 16)


def get_representation(value, wordvectors):
    if value in wordvectors:
        embedded_result = np.array(list(wordvectors[value]))
    else:
...
def get_str_representation(value, column, wordvectors, is_imdb=True):
    vec = np.array([])
    count = 0
    ...
    for v in value.split('%'):
        if len(v) > 0:
            if len(vec) == 0:
                vec = get_representation(prefix + v, wordvectors)
                count = 1
            else:
                new_vec = get_representation(prefix + v, wordvectors)
                vec = vec + new_vec
                count += 1
    if count > 0:
        vec /= float(count)
...
def encode_condition_op(condition_op, relation_name, index_name, parameters):
    ...
    is_imdb = parameters.is_imdb
    if parameters.wordvectors == None:
        wordvectors = load_dictionary(parameters.wordvectors_path)
        parameters.wordvectors = wordvectors
    else:
        wordvectors = parameters.wordvectors
    if condition_op is None:
        ...
        if re.match(r'.+\..+', left_value) is None:
            if relation_name is None:
                relation_name = IDX_TO_TABLE[index_name]
        ...
        if re.match(r'^[a-z][a-zA-Z0-9_]*\.[a-z][a-zA-Z0-9_]*$', right_value) is not None and right_value.split('.')[
            ...
            if right_value == "None": right_value = 0
            ...
            right_value_vec = get_str_representation(right_value, left_value, wordvectors, is_imdb).tolist()
            ...
            for v in right_value.split(','):
                ...
                if len(v) > 0:
                    count += 1
                    vec = get_str_representation(v, left_value, wordvectors, is_imdb).tolist()
        ...
        elif re.match(r'^__ALL__', right_value) is not None:
            operator_idx = parameters.compare_ops_id['!=']
            operator_vec = [0 for _ in range(parameters.compare_ops_total_num)]
            operator_vec[operator_idx - 1] = 1
            right_value = right_value.strip('\'')[7:].strip('{}')

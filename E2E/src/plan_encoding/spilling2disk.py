...
def make_data_job(plans, parameters):
    ...
    for o in operators_batch:
        if len(o) > max_nodes:
            max_nodes = len(o)
    # print(f'max # nodes per level in this batch : {max_nodes}')
    # print(f'max depth in this batch: {len(condition2s_batch)}')
    # raise
...
def save_data_job(plans, parameters, batch_size, directory, max_queries=-1):
    batch_id = 0
    if max_queries > 0 :
        plans = plans[0:max_queries]
    print('num queries = ' + str(len(plans)))
    for batch_id, plans_batch in enumerate(chunks(plans, batch_size)):
        ...
        np.save(directory + '/target_cost_' + str(batch_id) + '.np', target_cost_batch)
        np.save(directory + '/target_cardinality_' + str(batch_id) + '.np', target_cardinality_batch)
        np.save(directory + '/operators_' + str(batch_id) + '.np', operators_batch)
        np.save(directory + '/extra_infos_' + str(batch_id) + '.np', extra_infos_batch)
        np.save(directory + '/condition1s_' + str(batch_id) + '.np', condition1s_batch)
        np.save(directory + '/condition2s_' + str(batch_id) + '.np', condition2s_batch)
        np.save(directory + '/samples_' + str(batch_id) + '.np', samples_batch)
        np.save(directory + '/condition_masks_' + str(batch_id) + '.np', condition_masks_batch)
        np.save(directory + '/mapping_' + str(batch_id) + '.np', mapping_batch)
        print('saved: ', str(batch_id))

def get_data_job(plans, parameters, batch_size=64, max_queries=-1):
    data_batches = []
    batch_id = 0
    if max_queries > 0 :
        plans = plans[0:max_queries]
    for batch_id, plans_batch in enumerate(chunks(plans, batch_size)):
        ...
        target_cost_batch, target_cardinality_batch, operators_batch, extra_infos_batch, condition1s_batch, condition2s_batch, samples_batch, condition_masks_batch, mapping_batch = make_data_job(
            plans_batch, parameters)
        data_batches.append((target_cost_batch, target_cardinality_batch, operators_batch, extra_infos_batch, condition1s_batch, condition2s_batch, samples_batch, condition_masks_batch, mapping_batch))
    return data_batches
...
class Representation(nn.Module):
    def __init__(self, input_dim, hidden_dim, hid_dim, extra_info_size):
        ...
        self.lstm2 = nn.LSTM(15 + extra_info_size + 2 * hid_dim, hidden_dim, batch_first=True)
    ...
    def init_hidden(self, hidden_dim, batch_size=1, inputs=None):
    ...
    def forward(self, operators, extra_infos, condition1s, condition2s, samples, condition_masks, mapping):
        ...
        hidden = self.init_hidden(self.hidden_dim, num_level * num_node_per_level, inputs)

        # print(f'inputs device {inputs.device}')
        # print(f'hidden[0] device {hidden[0].device}')
        # print(f'hidden[1] device {hidden[1].device}')
        ...
        hidden = self.init_hidden(self.hidden_dim, num_node_per_level, last_level)
